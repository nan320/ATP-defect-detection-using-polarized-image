...
def compute_feature_maps(S0, DoLP, params):
    ...
    # 【核心修改点】Gap 信号提取
    # 1. DoLP 依然严格寻找低谷 (低于局部均值的部分)
    dolp_below = np.clip(dolp_mean - dolp_f32, 0, None)
    # 2. S0 不再预设阴影，而是寻找与周围存在“绝对差异”的显著变化区域（无论亮、暗）
    s0_absolute_deviation = np.abs(S0_f32 - s0_mean)

    # Overlap 信号提取
    s0_above   = np.clip(S0_f32 - s0_mean, 0, None)       # S0 局部更亮
...
PARAMS = dict(
    local_window         = 31,     # 局部均值滑窗大小
    gap_score_thresh     = 0.1,   # Gap 评分阈值 (0~1)，由于换成了绝对偏差，可以适当微调
    overlap_score_thresh = 0.10,   # Overlap 评分阈值 (0~1)
    morph_open           = 5,      # 开运算：消噪
    morph_close          = 5,      # 闭运算：填孔
    min_area             = 100,    # 连通域最小面积
    normal_dist          = 8,      # 排除正常束线边缘的距离阈值
    peak_distance        = 80,
    peak_prominence      = 15,
)
